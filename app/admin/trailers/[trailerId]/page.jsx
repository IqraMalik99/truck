...
export default function TrailerDetailPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/dashboard/trailer/${trailerId}/monthly-overview?year=${year}`, { cache: "no-store" });
      if (!res.ok) throw new Error(`Request failed (${res.status})`);
      const json = await res.json();
      setData(json);
    } catch (err) {
      setError(err.message || "Couldn't load trailer history.");
    } finally {
      setLoading(false);
    }
  }, [trailerId, year]);
  ...
  async function downloadPdf({ period, month, quarter, half, suffix }) {
    ...
    try {
      ...
      const res = await fetch(`/api/dashboard/trailer/${trailerId}/report?${qs.toString()}`);
      if (!res.ok) throw new Error("Couldn't generate report");
      const blob = await res.blob();
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = `trailer-${data.trailer.trailerNumber}-${year}-${suffix}.pdf`;
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14, paddingBottom: 18, borderBottom: "1px solid #e9ecf0", flexWrap: "wrap", gap: 12 }}>
        <div>
          <h1 style={{ fontSize: 21, fontWeight: 700, color: "#111827", margin: 0 }}>
            {data ? `Trailer ${data.trailer.trailerNumber}` : "Trailer"}
          </h1>
          <p style={{ fontSize: 12.5, color: "#6b7280", margin: "4px 0 0" }}>Monthly miles, drivers, trucks, and states covered.</p>
        </div>
              ...
              return (
                ...
                >
                  ...
                  <p style={{ fontSize: 11, color: "#6b7280", margin: "8px 0 0", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                    {m.trucks.length ? `Trucks: ${m.trucks.join(", ")}` : "No trucks"}
                  </p>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12.5 }}>
              <thead>
                <tr style={{ background: "#f9fafb" }}>
                  ...
                  <th style={{ textAlign: "left", padding: "8px 12px", color: "#6b7280", fontWeight: 600 }}>Truck</th>
              ...
              <tbody>
                {selected.trips.length === 0 && (
                  <tr>
                    <td colSpan={6} style={{ padding: "16px 18px", color: "#9ca3af" }}>No trips logged this month.</td>
                  </tr>
                )}
                {selected.trips.map((t, i) => (
                  <tr key={i} style={{ borderTop: "1px solid #f1f5f9" }}>
                    ...
                    <td style={{ padding: "8px 12px", color: "#374151" }}>{t.truck}</td>
